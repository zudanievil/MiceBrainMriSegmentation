...
def get_selected_structures(
    p: pathlib.Path,
) -> typing.Union[typing.Set[str], UniversumSet]:
    if not p.exists():
        return UniversumSet()
    else:
        with p.open("rt") as f:
...
def batches_gen(
    ...
    typing.Tuple[int, "numpy.ndarray[bool]", "numpy.ndarray[str]"]
]:
    """
...
def normalize_imgs(imgs, metas, spec) -> numpy.ndarray:
    ...
    hm = (
        metas[base_key].to_numpy()[..., numpy.newaxis, numpy.newaxis]
        if base_key
        else 0
    )
    std = (
        metas[var_key].to_numpy()[..., numpy.newaxis, numpy.newaxis]
        if var_key
        else 1
    )
    imgs = (imgs - hm) / std
...
def calculate_pixwise_t_ind(imgs, ref_mask) -> (numpy.ndarray, numpy.ndarray):
    ...
    return scipy.stats.ttest_ind(
        imgs[~ref_mask], imgs[ref_mask], axis=0, equal_var=False
    )
...
def compare(
    imgs, ref_mask, image_comparison_type: str, gaussian_kwargs
    ...
    if image_comparison_type == "independent":
        ...
        raise ValueError(
            "image_comparison_type must be one of {'independent', 'pairwise', 'none'}"
        )
...
) -> typing.Iterator[typing.Tuple[numpy.ndarray, dict]]:
...
def plot_pval_mdif(
    ...
    im1 = axs[1].imshow(
        mdif,
        cmap="coolwarm",
        norm=colors.TwoSlopeNorm(vcenter=0, vmin=None, vmax=None),
    )
...
def mean_std_npx_over_bool_mask(
    imgs: numpy.ndarray, bool_mask: "numpy.ndarray[bool]"
...
def main(
    ...
    """
    plt.ioff()
    srfi = info_classes.SegmentationResultFolderInfo.read(
        segmentation_result_folder_info
    )
    ...
    for batch_no, ref_mask, batch in bgen:
        ...
        pval, mdif = compare(
            imgs,
            metas["is_ref"],
            spec["image_comparison_type"],
            spec["pvalue_smoothing_kwargs"],
        )
        ...
        for structure_mask, structure_info in mask_gen(srfi, metas):
            ...
            stats.append(
                stats_over_structure_mask(
                    spec["pval_thresholds"],
                    pval,
                    imgs,
                    metas,
                    structure_mask,
...
def zero_self_comparison_stats(
    table: pandas.DataFrame, pvalue_thresholds: typing.Tuple[float]
) -> pandas.DataFrame:
    """
