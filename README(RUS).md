...
1. Если папка с масками еще не создана, нужно [создать](#Стандартный-способ-работы-с-масками:).
1. Запустить `scripts/segmentation.py`
1. Запустить `plot_segmentation_summary.py`, если нужно

#### Стандартный способ работы с масками:
1. Запустить скрипт `scripts/new_masks_folder.py` стандартным способом.
1. Скрипт создает таблицу `{masks_folder}/slice_ids.txt`, к-рая содержит список номеров срезов для атласа,
указанного в `lib/default_config.yml`, условно сопоставленных с координатами. С помощью этой таблицы (или без)
необходимо выяснить номера срезов и скачать их в папку `{masks_folder}/svgs/` вручную
либо с помощью скрипта `scripts/download_svgs.py`.
1. Названия срезов атласа должны совпадать с названиями срезов в именах изображений.
1. В `.svg` файлы срезов можно добавлять различные объекты (растровые изображения, линии).
