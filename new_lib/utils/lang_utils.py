...
def list_substructures(root: 'ElementTree.Element',
                       names: 'list[str]') -> 'list[ElementTree.Element]':
    result = []
    for name in names:
        found = False
...
def remove_node_xml_from_the_tree(root: ElementTree.Element, node: ElementTree.Element):
    for p in root.iter():
        if node in list(p):
            p.remove(node)
...
def assert_path_is_short(p):
    if len(str(p)) > 100 and sys.platform.startswith('win'):
        raise AssertionError(f'{p} is longer than 100 chars, paths longer than 256 chars may '
