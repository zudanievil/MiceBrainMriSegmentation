...
def stats_over_structure_mask(spec, pval, mdif, metas, structure_mask, structure_info) -> pandas.DataFrame:
    stat = metas.copy()
    not_ref = ~ metas['is_ref']
    stat['group'] = metas['group'][not_ref].iloc[0]
    stat['structure'] = structure_info['name']
    for t in spec['pval_thresholds']:
        out = mean_std_npx_over_bool_mask(mdif, (pval < t) * structure_mask)
...
def main(segmentation_result_folder_info: info_classes.segmentation_result_folder_info_like,
    ...
    for batch_no, ref_mask, batch in batches_gen(srfi, batch_range):
        ...
        for structure_mask, structure_info in mask_gen:
            ...
            a = spec, pval, mdif, metas, structure_mask, structure_info
            if save_intersection_images:
                plot_pval_mdif(*a, plot_folder)
            stats.append(stats_over_structure_mask(*a))
