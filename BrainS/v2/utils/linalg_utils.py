    ...
    def __call__(self, height: int, width: int) -> numpy.ndarray:
        ...
        return numpy.array(
            [
                a,
                b + shear_x,
                (scale_x - a) * center_x - b * center_y + trans_x,
                -b,
                a + shear_y,
                (scale_y - a) * center_y + b * center_x + trans_y,
                pan_x,
                pan_y,
                1,
            ]
...
def bbox_to_coords(
    y0: float, y1: float, x0: float, x1: float, non_cycled=True
) -> numpy.ndarray:
    """
    ...
    """
    coo = numpy.array(
        [[x0, y1], [x0, y0], [x1, y0], [x1, y1], [x0, y1]], dtype=numpy.float
    )
    if non_cycled:
        coo = coo[:-1]
    return coo
...
def rotate_coords(
    ...
    center: "numpy.ndarray or tuple or int",
    angle: int,
    degrees=False,
) -> numpy.ndarray:
    """
...
def sigmoid_with_quantiles(
    img: numpy.ndarray,
    sigmoid_gain: int = 10,
    sigmoid_cutoff: float = 0.5,
    quantiles: (float, float) = (0.07, 0.93),
) -> numpy.ndarray:
    flat = img.flatten()
...
def estimate_affine(
    src_coo: numpy.ndarray, dst_coo: numpy.ndarray
) -> numpy.ndarray:
