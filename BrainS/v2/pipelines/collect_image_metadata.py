...
def read_metadata_chuncks(
    fname_start: pathlib.Path, metadata_keys: dict, not_found_list: list
) -> dict:
...
def main(
    img_folder: info_classes.image_folder_info_like,
) -> "list[pathlib.Path]":
    """
    ...
    for fname in fname_gen(img_folder):
        # print(fname)
        meta = read_metadata_chuncks(
            meta_source_folder / fname, metadata_keys, not_found_list
        )
