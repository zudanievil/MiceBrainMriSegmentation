...
def generate_reference_table(
    ...
    for image_info in image_folder_info:
        ...
        chuncks = {
            f: int(c) if c.isnumeric() else c
            for f, c in zip(fname_fields, chuncks)
        }
        chuncks["name"] = image_info.name()
        ontology_info = ontology_folder_info.ontology_info(chuncks["frame"])
        brain_mask, head_mask = segment_head_and_brain_subpipeline.main(
            image_info, ontology_info
        )
...
def get_metrics_over_masks(
    ...
    result["histogram bg peak"] = mri_bg_peak(
        img
    )  # no reason to use it with masks
    result["head median over bg std"] = (
        result["head median"] / result["back std"]
    )
    return result
...
def mri_bg_peak(img: np.ndarray) -> float:
    ...
    """
    img_truncated = img[img < 0.3 * np.max(img)].astype(
        float
    )  # we truncate the long tail of high values
    hist_y, hist_x = ske.histogram(img_truncated, nbins=200)
    peak_idx = (
        hist_y[2:].argmax() + 2
    )  # sometimes 1st bin values 'outweigh' the gaussian
...
def fit_gaussian_on_histogram_peak(
    x: "np.ndarray[float]", y: "np.ndarray[float]", peak_idx: int
):
    """
    ...
    gthm = np.where(sks.flood(gthm.astype(int), peak_idx))[
        0
    ]  # for some reason does not work with bools
    ...
    idx = np.where(
        (x > mu_init - sigma_init * 2) & (x < mu_init + sigma_init * 2)
    )
    x = x[idx]
    y = y[idx]
    # fit
...
def plot_reference_table(
    ...
    for metric in metrics:
        ...
        t2 = t2.groupby(
            by=[*batch_spec["match_by"], *batch_spec["compare_by"]]
        ).mean()
        t2 = t2.unstack(batch_spec["compare_by"][0])

        plt.figure(figsize=(12, 10))
        x_values = t2.columns.get_level_values(
            batch_spec["compare_by"][0]
        ).to_numpy()
        for idx in t2.index:
