...
def main(image_info: info_classes.ImageInfo, ontology_info: info_classes.OntologyInfo) \
    ...
    brain_mask = brain_mask_to_image(open_root_mask(ontology_info), brain_coo, image.shape)
    head_mask = make_head_mask(image, brain_mask)
    return brain_mask, head_mask


def get_brain_coordinates(meta: dict, image_shape: tuple) -> "np.ndarray[float]":
    ...
    angle = - meta["rotation"]
    center = image_shape[1] / 2 - 0.5, image_shape[0] / 2 - 0.5  # horizontal, vertical
